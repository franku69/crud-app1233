...
def insert_into_tables(name, monthly_payment, costumer_type, username, password):
    try:
        ...
        cur.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (name, monthly_payment, costumer_type, datetime.now(), username, password))
        ...
        if costumer_type == 'member':
            ...
            cur.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s)
            """, (costumer_id, name, monthly_payment, datetime.now(), datetime.now() + timedelta(days=30)))
            mysql.connection.commit()
            ...
            cur.execute("""
                ...
                VALUES (%s, %s, %s, %s)
            """, (costumer_id, monthly_payment, datetime.now(), datetime.now() + timedelta(days=30)))
            mysql.connection.commit()
...
def dashboard():
    if 'username' in session and session.get('is_admin'):
        ...
        # Count of sales (assuming session is your sales table)
        ...
        return render_template('dashboard.html', visitors_count=visitors_count, members_count=members_count,
                               costumers_count=costumers_count, sales_count=sales_count)
    else:
        flash("You need to log in as an admin first.")
        return redirect(url_for('home'))
